...
class payment extends CI_Controller
{
	...
	{
	    $data=$this->user->showcart($this->session->userdata('uid'));
                                    ...
                                    if ($cart = $this->cart->contents())
                        				{
                        					foreach ($cart as $item)
                        					{
                        						...
                        						$cost=$details1[0]->cost;
                        						$data=array('productid'=>$postid,'uid'=>'10000','item'=>$qty,'attributevalue'=>$attributevalue,'cost'=>$cost,'txnid'=>$txnid,'status'=>$status,'dpo'=>$dpo,'orderid'=>$orderid);
                                    ...
                                    if ($cart = $this->cart->contents())
                        				{
                        					...
                        					{$details=$this->user->get_product_by_id($item['id']);
                        						...
                        						$qty=$item['qty'];
                        						$attributevalue=$item['attributevalue'];
                        						$dpo= date("Y-m-d H:i:s");	$cost=$details1[0]->cost;
                        						$data=array('productid'=>$postid,'uid'=>'10000','item'=>$qty,'attributevalue'=>$attributevalue,'cost'=>$cost,'txnid'=>$txnid,'status'=>$status,'dpo'=>$dpo,'orderid'=>$orderid);
						                        $this->db->insert('itemorder',$data);
                        			    	}
                                    ...
                                    if ($cart = $this->cart->contents())
                        				{
                        					...
                        					{$details=$this->user->get_product_by_id($item['id']);
                        						...
                        						$attributevalue=$item['attributevalue'];
                        						$dpo= date("Y-m-d H:i:s");	$cost=$details1[0]->cost;
                        						$data=array('productid'=>$postid,'uid'=>'10000','item'=>$qty,'attributevalue'=>$attributevalue,'txnid'=>$txnid,'cost'=>$cost,'status'=>$status,'dpo'=>$dpo,'orderid'=>$orderid);
						                        $this->db->insert('itemorder',$data);
                        			    	}
